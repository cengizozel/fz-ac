...
static void fz_ac_scene_remote_update_temp(FzAcApp* app) {
...
void fz_ac_scene_remote_on_enter(void* context) {
    ...
    for(uint32_t i = 0; i < AC_BUTTON_COUNT; i++) {
        const RemoteButtonLayout* layout = &remote_layout[i];
    ...
    fz_ac_scene_remote_update_temp(app);
...
bool fz_ac_scene_remote_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom &&
       FZ_AC_EVENT_TYPE(event.event) == FzAcCustomEventTypeButtonSelected) {
        ...
        if(button < AC_BUTTON_COUNT && app->remote.signals[button].present) {
            notification_message(app->notifications, &sequence_blink_white_100);
            ac_ir_signal_send(&app->remote.signals[button]);
